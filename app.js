const $=id=>document.getElementById(id);let device,server,char,pts=[],pos={x:0,y:0,z:0},vel={x:0,y:0,z:0},last=0,running=false,lastStill=0,rx=[],bias={x:0,y:0,z:0},cal=[],segmentStart=null,segmentPath=0,live={ax:0,ay:0,az:0,gx:0,gy:0,gz:0,roll:0,pitch:0,yaw:0};
...
function packet(e){
  ...
  if(incoming.length===20 && incoming[0]===0x55 && incoming[1]===0x61){
    ...
    if(liveEl)liveEl.textContent='LIVE SENSOR • '+Math.hypot(live.ax,live.ay,live.az).toFixed(2)+' m/s²';
...
function integrate(ax,ay,az){
 ...
 const ox=pos.x,oy=pos.y,oz=pos.z;pos.x+=vel.x*dt;pos.y+=vel.y*dt;pos.z+=vel.z*dt;segmentPath+=Math.hypot(pos.x-ox,pos.y-oy,pos.z-oz);render()
}
$('start').onclick=()=>{pos={x:0,y:0,z:0};vel={x:0,y:0,z:0};pts=[{...pos}];segmentStart={...pos};segmentPath=0;last=0;lastStill=0;cal=[];bias={x:0,y:0,z:0};running=true;$('point').disabled=$('close').disabled=false;render()};
$('point').onclick=()=>{pts.push({...pos});segmentStart={...pos};segmentPath=0;render()};$('close').onclick=()=>{if(pts.length>2)pts.push({...pts[0]});render()};$('reset').onclick=()=>{running=false;pts=[];pos={x:0,y:0,z:0};vel={x:0,y:0,z:0};render()};
